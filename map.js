...
$(document).ready(function() {
    //1. fortwsi tou xarti me ta poligwna gia ton diaxiristi
    //2. epeksergasia poligwnwn
    $.ajax({
        url: 'mapdata.php',
        dataType: 'json',
        ...
        success: function(response) {
            if(response.error == true){
                $('#mapid').html('<h2>Σφάλμα φόρτωσης χάρτη και δεδομένων: <br>' + response.msg + '</h2>');
                ...
                for(i = 0; i < num; i++) { //gia ola ta poligwna na ginoun xrwma gkri
...
//Popup gia tin eisagwgi dedomenwn kampilwn zitisis tou kathe poligwnou
function popupBind(polygon) {
    ...
    var headerInfo = "<h3 id='header-info'> ΕΙΣΑΓΩΓΗ ΠΛΗΡΟΦΟΡΙΩΝ ΤΕΤΡΑΓΩΝΟΥ: " + polId + "</h3>";
    var template1 = "<h3 class='info'> ΘΕΣΕΙΣ ΣΤΑΘΜΕΥΣΗΣ: </h3>";
    var template2 = '<form id="form-1">\
  <label id="inp2" for="input2">Πλήθος θέσεων στάθμευσης: &nbsp' + numOfParkSpots  + '</label>\
  <input id="input2" class="input" type="number" min=0 max=1000 required /><br>\
  <button class="save-but" type="submit">Αποθήκευση</button></form><h3 id="result1" class="info"></h3><hr>\
  <form id="form-2"> <h3 class="info"> ΔΕΔΟΜΕΝΑ ΚΑΜΠΥΛΗΣ ΖΗΤΗΣΗΣ: </h3>';

    var template3 = '<div class="container-form-custom"><div id="buttons-top">\
  <button id="center-dem" type="button" onclick="preDemand(1)" >Κέντρο</button><button id="resid-dem" type="button" onclick="preDemand(2)" >Κατοικία</button><button id="const-dem" type="button" onclick="preDemand(3)" >Σταθερή</button></div>\
  <div class="block1"><div class="firstrow"><h4>Ώρα</h4><h4>Ποσοστό</h4></div>\
  ...
  <div class="block2"><div class="firstrow"><h4>Ώρα</h4><h4>Ποσοστό</h4></div>\
  ...
  <div id="buttons1"> <button class="demandButton" onclick="loadDemand(polId)" type="button"> Φόρτωση Αποθήκευμενων Δεδομένων Ζήτησης</button></div>\
  <div id="buttons2"> <button type="submit"> Αποθήκευση </button>\
  <button type="reset" value="Reset"> Εκκαθάριση Φόρμας </button></div>\
